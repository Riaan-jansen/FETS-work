'''module containing all the functions needed for spec_analysis notebook.
contains:
class: file_data;
functions, contains gauss;
dicts, nuclear data;
gauss fit, guess fit and slicer, for slicing around an area
seeing if it is peak and fitting a gaussian to it.'''
import numpy as np
...
def gauss_fit(x, y, guess=None):
    ...
    Returns: fit (y values), and optimised parameters (popt).'''
...
def guess_fit(x, y, guess_0=None, guess_2=7):
    ...
    if guess_0 is None:
        ...
        guess_index = nearest_index(x, guess_0)
...
def slicer(x, y, guess):
    ...
    +/- range = 0.'''
    width = int(guess[2])+2
    close_index = nearest_index(x, guess[0])
    x_slice = x[close_index-width:close_index+width]
    y_slice = y[close_index-width:close_index+width]
    return x_slice, y_slice


def nearest_index(x, target: float):
    '''takes a list like and a target value. returns index of element in
    the list closest to target value.'''
    array = np.empty_like(x)
    for i, ele in enumerate(x):
        array[i] = np.abs(ele - target)
